...
pE <- function(par,path0,province,p.t,A,M){
  ...
  t = p.t[2] + (1:21)
  #lambda2 = 1/7 * log(100)
  #pt = data.frame(date = t,pt = p.E.2 - p.E.2/ 100 * exp(lambda2 * (1:7)))
  pt = data.frame(date = t,pt = rep(max(p.seq),21))
...
Coef.origin = data.frame(NULL)
for(i in 1:length(provinces)){
  ui.1 = matrix(c(1,0,0,0),ncol = 1)#constraint matrix
  ui.2 = matrix(c(0,1,0,-1),ncol = 1)
  ui.3 = matrix(c(0,0,-1,1),ncol = 1)
  ui = cbind(ui.1,ui.2,ui.3)
  ci = c(0,1/21,-1/2,0)
  print(i)
  ...
  A = read.csv("/Users/gujia/Documents/统计局/Data/year2020_Wuhan.csv")%>%mutate(province = as.character(province)) %>% filter(province == province.temp) %>% mutate(date = ymd(date))
  if(province.temp %in% c("Anhui","Chongqin","Fujian","Hunan","Jiangxi","Shaanxi")){
    #op = optim(par = c(1/10000,1/14),pE,path0 = path0,province = province.temp,p.t = p.t, A = A,M = population)
    op = constrOptim(theta = c(1/1000,1/11.5,1/7),pE,grad = NULL,ui = ui,ci = ci,path0 = path0,province = province.temp,p.t = p.t, A = A,M = population)
    print(op$counts)
  }
  else{
    #op = optim(par = c(1/100,1/14),pE,path0 = path0,province = province.temp,p.t = p.t, A = A,M = population)
  op = constrOptim(theta = c(1/50,1/11.5,1/7),pE,grad = NULL,ui = ui,ci = ci,path0 = path0,province = province.temp,p.t = p.t, A = A,M = population)
  print(op$counts)
  }
  Gof = c(Gof,op$value)
  fitting.results = fitting(op$par,path0 = path0,province = province.temp,p.t = p.t, A = A, M = population)
  ...
  alpha0 = c(alpha0,op$par[2])
  alpha1 = c(alpha1,op$par[3])
  R0 = 5.7
  p.seq = percentage / 100 * exp(R0 / 14 * 1:14)
  A.seq = A %>% filter(date >= p.t[1],date <= p.t[2]) %>% dplyr::select(daily_sum)
  n = sum(A.seq * p.seq)
  E = c(E,n)
  A.seq.1 = A %>% filter(date > p.t[2],date <= p.t[2] + 21) %>% dplyr::select(daily_sum)
  #lambda2 = 1/7 * log(100)
  #p.seq.1 = percentage.2 - percentage.2 / 100 * exp(lambda2 * (1:7))
  p.seq.1 = rep(max(p.seq),21)
  Import.afterlockdown = c(Import.afterlockdown,sum(A.seq.1 * p.seq.1))
  pt.all = c(pt.all,percentage)
  #pt.2 = c(pt.2,percentage.2)
}
summary.pt = data.frame(province =provinces,E = E,N = N,pt = pt.all,alpha0 = alpha0,alpha1 = alpha1)
...
summary.pt
xtable(summary.pt[,c(1,8,2,3)])
xtable(summary.pt[,c(1,4,5,6,7)],digits = 5)
